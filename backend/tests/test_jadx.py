"""JADX adapter tests. Uses fake executables; does not require a real JADX install."""
...
from app.scanners.tools.jadx import (
    VERSION_ARGS,
    JadxTool,
    jadx_definition,
    jadx_source_dir,
...
sample = outdir / "com" / "example" / "Main.java"
...
if outdir is None:
    raise SystemExit(2)
outdir.mkdir(parents=True, exist_ok=True)
(outdir / "Main.java").write_text("class Main {}", encoding="utf-8")
"""
...
def test_jadx_definition_properties() -> None:
    definition = jadx_definition(executable="jadx")
    assert definition.name == "jadx"
    assert definition.executable == "jadx"
    assert definition.version_args == VERSION_ARGS
    assert definition.version_args == ("--version",)


@pytest.mark.asyncio
async def test_jadx_available_and_unavailable(tmp_path: Path) -> None:
    fake = _write_fake(tmp_path / "jadx", FAKE_SUCCESS)
    available = JadxTool(_settings(tmp_path, fake))
    missing = JadxTool(Settings(workspace_dir=tmp_path / "ws", jadx_bin=str(tmp_path / "missing-jadx")))
...
@pytest.mark.asyncio
async def test_jadx_version_success_and_failure(tmp_path: Path) -> None:
    good = JadxTool(_settings(tmp_path, _write_fake(tmp_path / "jadx-ok", FAKE_SUCCESS)))
    bad = JadxTool(_settings(tmp_path, _write_fake(tmp_path / "jadx-bad", FAKE_VERSION_FAIL)))
    apk = write_apk(tmp_path / "app.apk")
    ok = await good.run(_context(tmp_path, apk, scan_id="v-ok"))
    assert ok.version == "1.5.0-appprobe-test"
...
@pytest.mark.asyncio
async def test_jadx_successful_execution(tmp_path: Path) -> None:
    fake = _write_fake(tmp_path / "jadx", FAKE_SUCCESS)
    apk = write_apk(tmp_path / "app.apk")
    context = _context(tmp_path, apk)
    result = await JadxTool(_settings(tmp_path, fake)).run(context)
    ...
    assert result.findings == []
    output = context.workspace.tool_dir("jadx") / "output"
    assert output.is_dir()
    assert any(output.rglob("*.java"))
    assert result.extras["java_file_count"] == 1
    assert (context.workspace.tool_dir("jadx") / "jadx-meta.json").is_file()
    meta = json.loads((context.workspace.tool_dir("jadx") / "jadx-meta.json").read_text(encoding="utf-8"))
    assert meta["java_file_count"] == 1
    assert jadx_source_dir(result.output_dir) == output


@pytest.mark.asyncio
async def test_jadx_nonzero_exit(tmp_path: Path) -> None:
    fake = _write_fake(tmp_path / "jadx", FAKE_FAIL)
    apk = write_apk(tmp_path / "app.apk")
    result = await JadxTool(_settings(tmp_path, fake)).run(_context(tmp_path, apk))
...
@pytest.mark.asyncio
async def test_jadx_timeout(tmp_path: Path) -> None:
    fake = _write_fake(tmp_path / "jadx", FAKE_SLEEP)
    apk = write_apk(tmp_path / "app.apk")
    result = await JadxTool(_settings(tmp_path, fake, timeout=1)).run(_context(tmp_path, apk))
...
@pytest.mark.asyncio
async def test_jadx_missing_output_is_failure(tmp_path: Path) -> None:
    fake = _write_fake(tmp_path / "jadx", FAKE_EMPTY)
    apk = write_apk(tmp_path / "app.apk")
    result = await JadxTool(_settings(tmp_path, fake)).run(_context(tmp_path, apk))
    assert result.status is ToolStatus.AVAILABLE_BUT_FAILED
...
@pytest.mark.asyncio
async def test_jadx_rejects_ipa_aab_and_arbitrary(tmp_path: Path) -> None:
    fake = _write_fake(tmp_path / "jadx", FAKE_SUCCESS)
    tool = JadxTool(_settings(tmp_path, fake))
...
@pytest.mark.asyncio
async def test_jadx_workspace_isolation_and_special_paths(tmp_path: Path) -> None:
    fake = _write_fake(tmp_path / "jadx", FAKE_ECHO_ARG)
    ...
    out_one = first.workspace.tool_dir("jadx")
    out_two = second.workspace.tool_dir("jadx")
...
@pytest.mark.asyncio
async def test_jadx_does_not_modify_apk(tmp_path: Path) -> None:
    fake = _write_fake(tmp_path / "jadx", FAKE_SUCCESS)
    apk = write_apk(tmp_path / "app.apk")
    before = apk.read_bytes()
    await JadxTool(_settings(tmp_path, fake)).run(_context(tmp_path, apk))
    assert apk.read_bytes() == before


def test_jadx_report_statuses() -> None:
    executed = ScanJob(
        id="rep",
        filename="app.apk",
        ...
        tool_runs=[
            ToolRunRecord(
                ...
                version="1.5.0-appprobe-test",
                reason="ok",
                output_dir="/tmp/scans/rep/tools/jadx",
            )
        ],
    )
    markdown = MarkdownReporter().render(executed)
    assert "| JADX | EXECUTED | 1.5.0-appprobe-test |" in markdown
    assert "class Main" not in markdown
    missing = ScanJob(
        id="rep2",
        filename="app.apk",
        artifact_path="app.apk",
        ...
        artifact_kind=ArtifactKind.APK,
        status=ScanStatus.COMPLETED,
        tool_runs=[ToolRunRecord(name="jadx", status=ToolStatus.NOT_AVAILABLE, reason="missing")],
    )
    timeout_job = ScanJob(
        id="rep3",
        filename="app.apk",
        ...
        status=ScanStatus.COMPLETED,
        tool_runs=[
            ToolRunRecord(name="jadx", status=ToolStatus.TIMEOUT, version="1.5.0-appprobe-test")
        ],
    )
    assert "| JADX | NOT AVAILABLE | - |" in MarkdownReporter().render(missing)
    assert "| JADX | TIMEOUT | 1.5.0-appprobe-test |" in MarkdownReporter().render(timeout_job)
